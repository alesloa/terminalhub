...
- **Universal attention detection** — you don't need a per-agent bell hook to get notified. Three
  ...
  terminal that went quiet after working). A **Settings → Notifications → "Detect agent attention by"**
  picker chooses the mode: **Layered** (default — bell/notify codes *and* silence, never miss),
  **Explicit only** (bell/notify codes — zero false alarms), or **Silence only** (works for any tool,
  even ones that signal nothing). A **Quiet window** stepper (default 10s) tunes how long silent counts
  as finished. Closed rooms use tmux's bell + silence flags; open rooms catch the bell, OSC codes, and a
  client-side idle timer — so the same notification fires either way. Makes the Claude bell hook optional.
